  ...
  >
    ...
    <!-- <template #east="props">
      <d3-text
        ...
        :x="3"
        :y="props.scale(city.values[city.values.length - 1].temperature)"
        dy="0.35em"
      >
        {{ city.id }}
    </d3-text>
    ...
    <template #west="props">
      <d3-axis orientation="Left" title="Distance, cm" v-bind="props" />
    </template>
  </d3-cartesian>
</template>
...
import { Cartesian, Line, Axis } from "./d3-vue";
const parseTime = d3.timeParse("%M%S.%L"); // "%Y%m%d");
...
export default {
  ...
  created() {
    d3.tsv("/data/temp.csv", (d) => { // , _, columns) => {
      console.log("d", d);
    ...
    //     d[(c = columns[i])] = +d[c];
      ...
      //       id: id,
      //       values: data.map((d) => {
      //         return { timestamp: d.timestamp, distancecm: d[id] };
      //       }),
      //     };
      //   });
      this.x.domain = d3.extent(data, (d) => d.timestamp);
      this.y.domain = [0, d3.max(data, (d) => d.distancecm)]; // d3.extent(data, (d) => d.distancecm);
      //   [
